...
const [productsRes, ticketsRes] = await Promise.all([
  axiosData("GET", `/api/products?populate=*&pagination[pageSize]=${pageSize * 0.7}`),
  axiosData("GET", `/api/tickets?populate=*&filters[publishedAt][$notnull]=true&pagination[pageSize]=${pageSize * 0.3}`)
]);
```
