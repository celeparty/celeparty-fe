...
type VariantItemProps = {
  index: number;
  control: Control<iTicketFormReq>;
  register: UseFormRegister<iTicketFormReq>;
  onRemove: () => void;
  errors?: any;
};

export const TicketVariantItem = ({
  index,
  register,
  onRemove,
  errors,
  ...
  const getFieldName = <T extends FieldPath<iTicketVariant>>(
    field: T
  ): `variant.${number}.${T}` => `variant.${index}.${field}`;
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="input-group md:col-span-4">
          ...
          {errors?.variant?.[index]?.name && (
            <p className="text-red-500 text-xs mt-1">
              {errors.variant[index].name.message}
            </p>
          )}
        </div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <input
                ...
                value={
                  field.value
                    ? formatNumberWithDots(field.value.toString())
                    : ""
                }
                onChange={(e) => {
        ...
        <div className="input-group md:col-span-4">
          ...
          {errors?.variant?.[index]?.quota && (
            <p className="text-red-500 text-xs mt-1">
              {errors.variant[index].quota.message}
            </p>
          )}
        </div>
      </div>
