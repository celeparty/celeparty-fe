...
export const TicketSend: React.FC = () => {
	...
	const [recipients, setRecipients] = useState<iRecipient[]>([{ name: "", email: "", phone: "" }]);
		...
		mutationFn: async () => {
			...
			const payload = {
				data: {
					...
					total_price:
						(selectedProductData?.variants?.find(
							(v: { name: string; price: number; stock: number }) => v.name === selectedVariant,
						)?.price || 0) * quantity,
