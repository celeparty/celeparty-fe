...
export const TicketScan: React.FC = () => {
	...
	const verifyTicketMutation = useMutation({
		mutationFn: async (ticketCode: string) => {
			const response = await axiosUser("PUT", `/api/qr-verify`, `${session?.jwt}`, { ticket_code: ticketCode });
			return response;
		},
		onSuccess: () => {
			...
			<Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>Informasi lengkap tiket yang dipindai</DialogDescription>
					</DialogHeader>
					{ticketDetail && (
