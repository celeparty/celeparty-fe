...
export const ProfileForm = () => {
	...
	useEffect(() => {
		if (status === "authenticated") {
			axiosUser("GET", `/api/users/${session?.user.id}?populate=*`, `${session && session?.jwt}`).then((res) => {
				setMyData(res);
			});
		}
	}, [status]);
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			...
			const response = await axiosUser("PUT", `/api/users/${myData.id}`, `${session && session?.jwt}`, reqData);
