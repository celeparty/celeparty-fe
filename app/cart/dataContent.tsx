                        ...
                        <div className="mt-6">
                          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <h5 className="text-lg font-bold text-blue-800">Detail Penerima Tiket</h5>
                            </div>
                            <p className="text-sm text-blue-600 mb-4">
                              Lengkapi data penerima untuk setiap tiket yang akan dibeli. Pastikan data sesuai dengan identitas asli.
                            </p>
                          </div>
                          ...
                          <div className="space-y-4">
                            {Array.from({ length: item.quantity }, (_, idx) => {
                              ...
                              const currentRecipient = item.recipients?.[idx] || {};
                              const hasErrors = !currentRecipient.name || !currentRecipient.identity_type ||
                                               !currentRecipient.identity_number || !currentRecipient.whatsapp_number || !currentRecipient.email;
                                ...
                                <div key={idx} className={`border rounded-lg p-4 transition-all duration-200 ${
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                                          !currentRecipient.name ? 'border-red-300 bg-red-50' : 'border-gray-300'
                                        }`}
                                        value={currentRecipient.name || defaultName}
                                        onChange={(e) => {
                                          const newRecipients = [...(item.recipients || [])];
                                          if (!newRecipients[idx]) newRecipients[idx] = {
                                            name: defaultName,
                                            identity_type: defaultIdentityType,
                                            identity_number: defaultIdentityNumber,
                                      ...
                                      {!currentRecipient.name && (
                                        <p className="text-xs text-red-600">Nama lengkap wajib diisi</p>
                                      )}
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <select
                                        className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                                          !currentRecipient.identity_type ? 'border-red-300 bg-red-50' : 'border-gray-300'
                                        }`}
                                        value={currentRecipient.identity_type || defaultIdentityType}
                                        onChange={(e) => {
                                          const newRecipients = [...(item.recipients || [])];
                                          if (!newRecipients[idx]) newRecipients[idx] = {
                                            name: defaultName,
                                      ...
                                      {!currentRecipient.identity_type && (
                                        <p className="text-xs text-red-600">Tipe identitas wajib dipilih</p>
                                      )}
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                                          !currentRecipient.identity_number || !/^\d+$/.test(currentRecipient.identity_number)
                                            ? 'border-red-300 bg-red-50' : 'border-gray-300'
                                        }`}
                                        value={currentRecipient.identity_number || defaultIdentityNumber}
                                        onChange={(e) => {
                                          const newRecipients = [...(item.recipients || [])];
                                          if (!newRecipients[idx]) newRecipients[idx] = {
                                            name: defaultName,
                                          ...
                                          newRecipients[idx].identity_number = e.target.value.replace(/\D/g, '');
                                          updateRecipients(item.product_id, newRecipients);
                                        }}
                                      />
                                      {(!currentRecipient.identity_number || !/^\d+$/.test(currentRecipient.identity_number)) && (
                                        <p className="text-xs text-red-600">Nomor identitas wajib diisi dengan angka</p>
                                      )}
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <input
                                        ...
                                        className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                                          !currentRecipient.whatsapp_number || !/^\d+$/.test(currentRecipient.whatsapp_number)
                                            ? 'border-red-300 bg-red-50' : 'border-gray-300'
                                        }`}
                                        value={currentRecipient.whatsapp_number || defaultWhatsappNumber}
                                        onChange={(e) => {
                                          const newRecipients = [...(item.recipients || [])];
                                          if (!newRecipients[idx]) newRecipients[idx] = {
                                            name: defaultName,
                                          ...
                                          newRecipients[idx].whatsapp_number = e.target.value.replace(/\D/g, '');
                                          updateRecipients(item.product_id, newRecipients);
                                        }}
                                      />
                                      {(!currentRecipient.whatsapp_number || !/^\d+$/.test(currentRecipient.whatsapp_number)) && (
                                        <p className="text-xs text-red-600">Nomor WhatsApp wajib diisi dengan angka</p>
                                      )}
                                    </div>
                                    ...
                                    <div className="space-y-1 md:col-span-2">
                                      ...
                                      <input
                                        ...
                                        className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors ${
                                          !currentRecipient.email || !/\S+@\S+\.\S+/.test(currentRecipient.email)
                                            ? 'border-red-300 bg-red-50' : 'border-gray-300'
                                        }`}
                                        value={currentRecipient.email || defaultEmail}
                                        onChange={(e) => {
                                          const newRecipients = [...(item.recipients || [])];
                                          if (!newRecipients[idx]) newRecipients[idx] = {
                                            name: defaultName,
                                  ...
                                  {hasErrors && (
                                    <div className="mt-3 p-2 bg-red-100 border border-red-200 rounded-md">
                                      <p className="text-xs text-red-700">
                                        ⚠️ Lengkapi semua field yang wajib diisi untuk melanjutkan pembayaran
                                      </p>
                                    </div>
                                  )}
                                </div>
                          ...
                          <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                <p className="text-sm font-medium text-green-800 mb-1">Informasi Penting</p>
                                <ul className="text-xs text-green-700 space-y-1">
                                  <li>• Pastikan data penerima sesuai dengan identitas asli</li>
                                  <li>• E-tiket akan dikirim ke email yang terdaftar</li>
                                  <li>• Setiap tiket memiliki barcode unik</li>
                                </ul>
                              </div>
                            </div>
                          </div>
