...
export default function Wallet() {
  ...
  const onSubmit = async (data: iMerchantProfile) => {
    ...
    const calculatedActiveBalance =
      formatMoneyReq(saldo_active) - formatMoneyReq(withdrawBalance);

    const calculatedRefundBalance =
      formatMoneyReq(saldo_refund) + formatMoneyReq(withdrawBalance);
  ...
  return (
    <>
      <Box className="lg:mt-0 mt-2">
        ...
        ></BankAccountDetails>
        <h4 className="text-black text-[14px] lg:text-[17px] font-extrabold mt-4">
          Tarik Dana
        </h4>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <ItemInput label="Tarik Saldo">
            ...
            />
            {errorBalance && (
              <p className="text-red-500 text-[10px] mt-2">Saldo tidak cukup</p>
            )}
          </ItemInput>
