...
const passwordSchema = z.object({
	email: z.string().nonempty({ message: "Email tidak boleh kosong" }).email({ message: "Alamat email tidak valid" }),
});
			...
			<div className="flex justify-between items-start gap-7">
				<Box className=" w-full max-w-[600px] mx-auto mt-0">
					...
					<form onSubmit={handleSubmit(sendForgot)} className="flex flex-col gap-4">
						...
						<div className="relative">
							<Button className="h-[42px] text-center text-white rounded-full px-7">Submit</Button>
						</div>
					</form>
					{message ? (
