...
const signUpSchema = z
  .object({
    ...
    companyName: z
      .string()
      .nonempty({ message: "Nama usaha tidak boleh kosong" }),
    serviceLocation: z
      .array(z.any())
      .nonempty({ message: "Lokasi layanan tidak boleh kosong" }),
    bankName: z.string().nonempty({ message: "Nama bank tidak boleh kosong" }),
    accountNumber: z
      .string()
      .nonempty({ message: "Nomor rekening tidak boleh kosong" }),
    accountName: z
      .string()
      .nonempty({ message: "Nama pemegang rekening tidak boleh kosong" }),
    password: z
      .string()
      ...
      <div className="relative">
        <form onSubmit={handleSubmit(signUp)} className="flex flex-col gap-4">
          ...
          <div className="relative">
            ...
            />
            {errors.nik && (
              <span className="text-red-500 text-xs">{errors.nik.message}</span>
            )}
          </div>
          <div className="relative">
            <input
          ...
          <div className="bg-[#553AA9] px-4 py-5 rounded-lg">
            ...
            {fields.map((item, index) => (
              <div
                key={item.id}
                className="relative flex flex-col lg:flex-row gap-2 items-center mb-5"
              >
                <select
                  className="text-black px-4 py-2 rounded-lg min-w-[270px] w-full"
